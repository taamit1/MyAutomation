...
pkg_ver=`echo ${olb_ear} | awk 'BEGIN { FS="_SSB_" }{ print $2 }'|awk -F"_" '{print $1}'

#### Copying packages to DMGR
arr[0]=/platform/${fiid}/EP/build/${olb_ear}
arr[1]=/platform/${fiid}/EP/build/${olb_jar}
...
scp  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${file2scp} ${cwd}/OLBsubprgm_deploy.ksh ${usrid}@${dmgr}:${stage}/ 2>&1

echo "###########################################################"
echo "# Starting the deployment of new build on DMGR ${srvr_nm} #"
echo "###########################################################"

ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "chmod 755 ${stage}/OLBsubprgm_deploy.ksh ; ${stage}/OLBsubprgm_deploy.ksh ${olb_ear} ${olb_jar} ${finame}" 2>&1
success_code=$?
...
then
        ...
        date
        echo "------------------------------------------------------"
        ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "rm ${stage}/OLBsubprgm_deploy.ksh"
