...
printf << Label >> ${cnfpth}/config.xml_tmp "    <listView>
      ...
      <jobNames class=\"tree-set\">
         ...
         <string>$SHORTNAME-$FIID-Pass3-Custom</string>
"
Label
...
cp -pr ${cwd}/new_env/Pass3-Custom ${jobpth}/${SHORTNAME}-${FIID}-Pass3-Custom

perl -p -i -e "s/SHORTNAME/${SHORTNAME}/g" ${jobpth}/${SHORTNAME}-${FIID}-Pass1/config.xml ${jobpth}/${SHORTNAME}-${FIID}-Pass2/config.xml ${jobpth}/${SHORTNAME}-${FIID}-Pass3-Custom/config.xml
...
/usr/java7_64/jre/bin/java -DJENKINS_HOME=/packages/jenkins -jar jenkins-cli.jar  -s http://192.168.186.51:9443/jenkins/ reload-configuration --username admin --password p@s5w0rd

else
	echo "Missing one or more Server Name / Server IP / Application User"
fi
