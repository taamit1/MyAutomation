...
echo "------------------------------------------------------"
sudo su - $dmsusr -c " ${cwd}/CIsubprgm_pass1.ksh ${fiid} ${dt}" 2>/dev/null
...
mail -r "dmsbuild@bankonline.com"  -s "Build Verification for ${fiid}" Ronald.Henry@aciworldwide.com < ${logpath}/tmp
