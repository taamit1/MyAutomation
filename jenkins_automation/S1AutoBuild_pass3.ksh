...
scp  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${file2scp} ${cwd}/subprgm_pass3.ksh ${usrid}@${dmgr}:${stage}/ 2>&1

echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo "+ Starting the deployment of new build on DMGR ${srvr_nm} +"
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"

ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "chmod 755 ${stage}/subprgm_pass3.ksh ; ${stage}/subprgm_pass3.ksh ${pkg_ver} ${fiid}"
success_code=$?
...
then
	...
	ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "rm ${stage}/subprgm_pass3.ksh"
	...
	echo "Build deployment failed for ${fiid}, check the logs on app server"
	ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "rm ${stage}/subprgm_pass3.ksh"
	echo "------------------------------------------------------";date;echo "------------------------------------------------------"
	exit 1
fi
