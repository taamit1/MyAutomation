...
crt_pkgpath="/platform/${fiid}/s1env/EP/build"
...
#userid=`echo $str | ksh`
#sudo su - $userid -c " ${cwd}/subprgm_pass2.ksh ${fiid} ${dt}"
...
file2scp=`ls /platform/fi14673/s1env/EP/build/*3711H0V94*ar | grep -E "Config|EP_SSB" |  tr "\n" " "`
...
echo "scp  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${file2scp} ${cwd}/subprgm_pass3.ksh ${usrid}@${dmgr}:${stage}/"
scp  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${file2scp} ${cwd}/subprgm_pass3.ksh ${usrid}@${dmgr}:${stage}/
#scp  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${cwd}/subprgm_pass3.ksh ${usrid}@${dmgr}:${stage}/
...
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
#srvr_nm=`ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "hostname"`
echo "ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} \"chmod 755 ${stage}/subprgm_pass3.ksh ; ${stage}/subprgm_pass3.ksh ${pkg_ver} ${fiid} ; rm ${stage}/subprgm_pass3.ksh\""
ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "chmod 755 ${stage}/subprgm_pass3.ksh ; ${stage}/subprgm_pass3.ksh ${pkg_ver} ${fiid} ; rm ${stage}/subprgm_pass3.ksh"
success_code=$?
...
echo "All,\nNew EAR/JAR has been deployed on (PLEASE SPECIFY ENV.) (${srvr}) as per the request.\n" > ${logpath}/rndnum
echo "Environemnt is available for testing. Please respond within 24 hours that you have reviewed the environment and fixes." >> ${logpath}/rndnum
...
echo "The following EAR/JAR is available for back out purpose located at (/platform/${fiid}/WebSphere/DeploymentManager/Stage):" >> ${logpath}/rndnum
