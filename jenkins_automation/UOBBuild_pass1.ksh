...
then
    ...
    echo " Usage: $0 <FIID> <DMSUSER> <EMail>"
    echo " Example: $0 fi9999 dms9999 $Email"
    exit 1
fi
...
echo "------------------------------------------------------"
sudo su - $dmsusr -c " ${cwd}/UOBsubprgm_pass1.ksh ${fiid} ${dt}" 2>/dev/null
...
mail -r "dmsbuild@bankonline.com"  -s "Delta files Verification for ${fiid}" ${devemail} < ${logpath}/tmp
