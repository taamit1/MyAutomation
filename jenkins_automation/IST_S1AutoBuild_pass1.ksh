...
sudo su - $dmsusr -c "${cwd}/IST_subprgm_pass1.ksh ${fiid} ${dt}" 2>/dev/null
...
elif grep "OB" $pkg_info >/dev/null 2>&1
then
  patch_rpt="${srcpath}/s1env/EP/build/Patch_OB${pkg_ver}_${fiid}.rpt"
else
...
if [ $? -eq 0 ]
then
  echo "BUILD FAILED, please check for ERROR in the build logs..."
  echo "-------------------------------------------------------------"
  exit 1
else
