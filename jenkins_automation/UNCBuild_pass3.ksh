...
pkg_ver=`awk -F"EP|CI|CB|TO|NA|UB|OB" '/Marking SVN builds version/ {gsub(/\./,"");print $2}' ${logpath}/build_pass2.log`
...
scp  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${file2scp} ${cwd}/UNCsubprgm_pass3.ksh ${usrid}@${dmgr}:${stage}/ 2>&1

echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo "+ Starting the deployment of new build on DMGR ${srvr_nm} +"
echo "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"

ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "chmod 755 ${stage}/UNCsubprgm_pass3.ksh ; ${stage}/UNCsubprgm_pass3.ksh ${pkg_ver} ${fiid}"
success_code=$?
...
then
	...
	ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "rm ${stage}/UNCsubprgm_pass3.ksh"
	echo "Check email for details"
	echo "------------------------------------------------------";date;echo "------------------------------------------------------"
else
	echo "Build deployment failed for ${fiid}"
	ssh  -o StrictHostKeyChecking=no -i ${cwd}/.ssh/build-key ${usrid}@${dmgr} "rm ${stage}/UNCsubprgm_pass3.ksh"
	echo "------------------------------------------------------";date;echo "------------------------------------------------------"
	exit 1
fi
...
#echo "Provide any issues encountered during testing.\n" >> ${logpath}/rndnum
