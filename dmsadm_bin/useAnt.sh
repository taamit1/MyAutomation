...
echo "Copying templates from $S1CUSTHOME/S1-INSTALL-INF ..."
for TNAM in `ls $S1CUSTHOME/S1-INSTALL-INF/build* 2>>/dev/null`
do
  BNAM=`basename $TNAM`
  cat $TNAM | sed -e "s/FINAME/$FINAME/g"|sed -e "s/FI_BLD_VER/$EP_VER_SHORT/g" |sed -e "s/BLD_VER/$EP_VER_CUR/g" >$BNAM
done
...
EP3.7.3*) PATH=/usr/java5/bin:$PATH
          ENVMGRLIB=/support/bin/envMgrCli5/lib
          echo "changing PATH for version $EP_VER_CUR"
          echo
;;

EP3.7.4*) PATH=/usr/java6_64/bin:$PATH
          ENVMGRLIB=/support/bin4.01/envMgrCli6/lib
          echo "changing PATH for version $EP_VER_CUR"
          echo
...
;;
  ...
  CI3.7*) PATH=/usr/java5/bin:$PATH
          ENVMGRLIB=/support/bin2.15c/envMgrCli5/lib
          echo "changing PATH for version $EP_VER_CUR"
          echo
  ;;
  NA3.7*) PATH=/usr/java5/bin:$PATH
          ...
          echo
  ;;
  CB4.0*) PATH=/usr/java6_64/bin:$PATH
          ENVMGRLIB=/support/bin4.01/envMgrCli6/lib
          echo "changing PATH for version $EP_VER_CUR"
          echo
  ;;
