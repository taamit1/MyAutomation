...
#
case "$BUILD_TASK" in
  database-all) # remove database related build files
             echo "Preparing build directory for database..."
             cd $S1BLDHOME
             rm -rf $S1BLDHOME/$PRODNAME/database
	     rm -rf $S1BLDHOME/$PRODNAME/S1-INSTALL-INF
