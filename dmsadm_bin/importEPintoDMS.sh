...
#####
# Script to import CORE version in Pune DMS (punist01)
#####
...
if test ! -f ${CORE_FILE}
then
   echo "Can't read ${CORE_JAR} file to be imported"
   exit 1
else
   jar xf ${CORE_FILE}.jar
fi

cd ${CORE_FILE}

INST_LOCATION="$HOME/../$EP_VER"
INST_WORKLOC="${CORE_JAR}"
...
then
  ...
  cd Banking
    ...
    rsync -cr $INST_WORKLOC/source/Banking/ ./
