            ...
            parallel {
                ...
                stage('ICE-XS') {
                    steps {
                        script {
                            try {
                                sh "ansible-playbook ${PLAYBOOK} -i ${INVENTORY} -f 5 -t patch_icexs_primary_${SITE} -e '{\"active_site\": ${ACTIVE_SITE}}' ${EXTRA_ARGS}"
                            } catch (icexs1_err) {
                ...
                stage('AGI') {
                    steps {
                        script {
                            try {
                                sh "ansible-playbook ${PLAYBOOK} -i ${INVENTORY} -f 5 -t patch_agi_primary_${SITE} ${EXTRA_ARGS}"
                            } catch (agi1_err) {
                                echo agi1_err.getMessage ()
                            }
                        }
                    }
                }
            ...
            parallel {
                ...
                stage('ICE-XS') {
                    steps {
                        script {
                            try {
                                sh "ansible-playbook ${PLAYBOOK} -i ${INVENTORY} -f 5 -t patch_icexs_secondary_${SITE} -e '{\"active_site\": ${ACTIVE_SITE}}' ${EXTRA_ARGS}"
                            } catch (icexs2_err) {
                ...
                stage('AGI') {
                    steps {
                        script {
                            try {
                                sh "ansible-playbook ${PLAYBOOK} -i ${INVENTORY} -f 5 -t patch_agi_secondary_${SITE} ${EXTRA_ARGS}"
                            } catch (agi2_err) {
                                echo agi2_err.getMessage ()
                            }
                        }
                    }
                }
