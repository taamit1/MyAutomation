...
EOT
	chmod +x "$REPO_PATH/.git/hooks/$HOOK_FILE"
}
...
extend_hook() {
	...
	if [ -d "$REPO_PATH/.git/hooks/${HOOK_FILE}.d" ]
	then
	...
	# Create folder
	mkdir "$REPO_PATH/.git/hooks/${HOOK_FILE}.d"

	# If hook exist, move it to the new folder
	if [ -f "$REPO_PATH/.git/hooks/$HOOK_FILE" ]
	then
		mv -v "$REPO_PATH/.git/hooks/$HOOK_FILE" "$REPO_PATH/.git/hooks/${HOOK_FILE}.d/${HOOK_FILE}.$(date +%s%N)"
	fi
